...
class AdWin2:
    ...
    def __insert_element(self, value):
        ...
        incremental_variance = self.window_len * pow(value - mean, 2) / (self.window_len + 1) if self.window_len > 0 else 0
    ...
    def __delete_element(self):
        ...
        variance_reduction = bucket_row.bucket_variance[0] + (
            deleted_count * self.window_len * pow(deleted_mean - self.window_sum / self.window_len, 2) / (deleted_count + self.window_len)
        )
